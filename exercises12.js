function countProfit(shoppers) {
    ...
    for(var i = 0; i < listBarang.length; i++){
        ...
        for(var j = 0; j < shoppers.length; j++){
            if(object.product === shoppers[j].product) {
                if(shoppers[j].amount <= object.leftOver){
                    object.shoppers.push(shoppers[j].name)
                    // jumlah = jumlah + shoppers[j].amount;
                    object.leftOver = object.leftOver - shoppers[j].amount;
                    object.totalProfit += shoppers[j].amount * listBarang[i][1];        
                }
            }
